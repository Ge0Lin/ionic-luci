...
-- Copyright 2015 Lars Boegild Thomsen <lth@bright-things.com>
...
local DDNS = require "luci.tools.modbusgw"		-- ddns multiused functions
local UTIL = require "luci.util"

MODBUS_MIN = "2015-03-0"	-- minimum version of service required

function index()
	local nxfs	= require "nixio.fs"		-- global definitions not available
	local sys	= require "luci.sys"		-- in function index()
	local modbusgw	= require "luci.tools.modbusgw"	-- ddns multiused functions
	local verinst	= modbusgw.ipkg_ver_installed("modbusgw")
	local verok	= modbusgw.ipkg_ver_compare(verinst, ">=", "2015-03-05")
	-- do NOT start it not ddns-scripts version 2.x
	if not verok then
		return
	end
	-- no config create an empty one
	if not nxfs.access("/etc/config/modbusgw") then
		nxfs.writefile("/etc/config/modbusgw", "")
	end
